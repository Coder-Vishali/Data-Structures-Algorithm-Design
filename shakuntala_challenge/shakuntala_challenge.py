...
if __name__ == '__main__':
    ...
    try:
        with open("inputPS13.txt", "r") as f:
            ...
            for index, eachline in enumerate(f.readlines()):
                    ...
                    if all([term.lower() in eachline.lower() for term in ["Number"]]):
                        # getting rid of any commas present in the text
                        eachline = re.sub(r",", "", eachline)
                        # the number can be both positive or negative number
                        m = re.search(r"Number\s*(\d+)\s*[:=-]?\s*(-?[0-9]+)", eachline, re.IGNORECASE)
                        if m:
                            # the second match of regex is the input number that needs to be multiplied
        ...
        print(f"FileNotFoundError: Missing \'inputPS13.txt\' file")
        sys.exit()
